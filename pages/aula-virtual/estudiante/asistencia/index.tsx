...
const AsistenciaAulaVirtual = () => {
  ...
  const { validateUserAulaVirutal, getDetailsofAttendanceAulaVirtual } =
    useAulaVirtual();
  ...
  useEffect(() => {
    ...
    if (
      `${router.query.dni}`.length > 0 &&
      `${router.query.idInstitucion}`.length > 0
    ) {
  ...
  return (
    <div className="">
      <div className="grid relative  h-[350px]  bg-headerPsicolinguistica overflow-hidden drop-shadow-lg">
        ...
        <div className="w-[60%] max-lg:w-[80%] max-xl:w-[80%] m-auto pt-20">
          <div className="grid items-center justify-start mb-20">
            <h1 className="text-textTitulos relative z-[20] text-3xl font-bold font-martianMono capitalize text-left">
              Asistencia de {dataAulavirtual.name} {dataAulavirtual.lastname}{" "}
              {dataAulavirtual.firstname}
            </h1>
          </div>
            ...
            <div className="m-auto w-full">
              ...
              <div className="flex justify-between">
                ...
                <div
                  ...
                  className=" rounded-md bg-gradient-to-r from-colorTercero to-colorSecundario text-center p-3 h-[50px] font-semibold text-white cursor-pointer capitalize drop-shadow-lg"
                >
                  descargar pdf
                </div>
                ...
                <select
                  className="p-3 bg-white rounded-md xm:w-[200px] mb-5 text-slate-400 drop-shadow-lg uppercase outline-none"
                  onChange={handleChangeValueMonth}
                  name="month"
                >
      ...
      <div className=" w-[90%] xm:w-[80%] m-auto pt-10 xl:w-[60%]">
        ...
        <table ref={pdfRef} className="w-[100%]  shadow-md m-auto mb-10">
          ...
          <thead className=" border-b-2 border-gray-200 bg-gradient-to-r from-colorTercero to-colorSecundario">
            <tr className="text-textTitulos font-martianMono capitalize ">
              <th className="py-3 md:p-2 pl-1 md:pl-2 text-sm text-center uppercase">
                fecha
              </th>
              <th className="py-3 md:p-2 text-sm text-center uppercase">dia</th>
              <th className="py-3 md:p-2 text-sm text-center uppercase">
                ingreso
              </th>
              <th className="py-3 md:p-2 text-sm text-center uppercase">
                salida
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {resumeAttendanceStudent?.map((item) => {
              return (
                <tr
                  key={item?.date}
                  className="text-slate-500 h-[40px] hover:bg-hoverTableSale duration-100 cursor-pointer w-full"
                >
                  <td className="text-sm text-center font-dmMono">
                    {item?.date}
                  </td>
                  <td className="uppercase text-sm text-center font-dmMono">
                    {item?.day}
                  </td>
                  <td className="text-center text-sm font-dmMono">
                    {item?.attendance}
                  </td>
        ...
        {resumeAttendanceStudent.length > 0 ? null : (
          <>
            <p className="text-center p-5 text-slate-400">
              no existe registros de este mes
            </p>
          </>
        )}
      </div>
