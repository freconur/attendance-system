...
const ResumenConsulta = () => {
  ...
  const { employeeData, userData, resumenAttendanceEmployee } = useGlobalContext()
  const { dataEmployeeConsulta } = useNavbarSearch()
  const { getDetailAttendanceEmployee, getDetailAttendanceEmployeeConsultas } = useAttendanceEmployee()
  const paramMonth = searchParams.get('mes')
  const paramDni = searchParams.get('dni')
  const paramIdInstitution = searchParams.get('id')
  useEffect(() => {
    dataEmployeeConsulta(`${paramDni}`, `${paramIdInstitution}`)
    getDetailAttendanceEmployeeConsultas(`${paramDni}`, `${paramMonth}`, `${paramIdInstitution}`)
      ...
      <div className='w-full'>
        ...
        <div className='w-full m-auto hidden md:block'>
          ...
          <div className=' m-auto flex justify-between w-[100%] md:w-[90%] '>
            <h3 className='text-xl text-slate-500 uppercase text-left mb-5'>asistencia de <span className='font-semibold'>{employeeData?.name} {employeeData?.lastname} {employeeData?.firstname}</span></h3>
