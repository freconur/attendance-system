...
const useAttendanceEmployee = () => {
  ...
  const URL_API_GOOGLE =
    "https://api-attendance-production-6be0.up.railway.app";
  ...
  const employeeModal = (value: boolean) => {
    dispatch({
      type: AttendanceRegister.ACTIVE_EMPLOYEE_MODAL,
      payload: !value,
    });
  };
          ...
          .then(async (res) => {
            if (res.data.estado) {
              ...
              await setDoc(
                doc(
                  db,
                  `/intituciones/${userData.idInstitution}/usuarios`,
                  `${data.dni}`
                ),
                employee
              );
              await setDoc(doc(db, `users`, `${data.dni}`), employee).then(
                (rta) => {
                  resolve(true);
                }
              );
  ...
  const getTypeEmployee = async () => {
    ...
    const querySnapshot = await getDocs(
      collection(db, `/intituciones/${userData.idInstitution}/types-employee`)
    );
    ...
    dispatch({
      type: AttendanceRegister.GET_TYPES_EMPLOYEE,
      payload: typesEmployee,
    });
  };
  ...
  const getDataEmployeeByRol = async (employees: Employee[], date: string) => {
    ...
    await Promise.all(
      employees.map(async (employee) => {
        ...
        const newData = {
          ...employee,
          attendanceByDate: attendanceEmployee.exists()
          ...
          departureByDate: attendanceEmployee.exists()
            ? attendanceEmployee?.data().departureTime
              ? hoursUnixDate(attendanceEmployee?.data().departureTime)
              : "sin registro"
            : "sin registro",
        };
  ...
  const getEmployeeAndAttendance = async (employee: string) => {
    ...
    if (employeeData.exists()) {
      ...
      if (employeeAttendanceData.exists()) {
        //verificando si el usuario ya registro su ingreso para luego condicionar si registrara el ingreso o salida
        if (
          employeeAttendanceData.data().arrivalTime &&
          employeeAttendanceData.data().departureTime === undefined
        ) {
          ...
          const validateDataEmployee = {
            hour: hourAttendanDeparture.getHours(),
            min: hourAttendanDeparture.getMinutes(),
          };
          const rta = validateDepartureTime(
            employeeAttendanceData.data()?.arrivalTime,
          ...
          await setDoc(arrivalTimeRef, {
            arrivalTime: hourAttendanDeparture,
            manualAttendance: true,
          });
          dispatch({
            type: AttendanceRegister.LOADER_GET_EMPLOYEE,
            payload: false,
          });
      ...
      dispatch({
        type: AttendanceRegister.GET_EMPLOYEE,
        payload: {
          ...employeeData.data(),
          currentlyHour: currentlyHour(hourAttendanDeparture),
        },
      });
      dispatch({
        type: AttendanceRegister.LOADER_GET_EMPLOYEE,
  ...
  const getDetailAttendanceEmployee = async (dni: string, month: string) => {
    ...
    if (arrivalTimeFromEmployee) {
      ...
      dispatch({
        type: AttendanceRegister.GET_EMPLOYEE_RESUME_ATTENDANCE,
        payload: arrivalTimeFromEmployee,
      });
    }
  };

  const getDetailAttendanceEmployeeConsultas = async (
    dni: string,
    month: string,
    id: string
  ) => {
    const querySnapshot = await getDocs(
      collection(
        db,
        `/intituciones/${id}/attendance-employee/${dni}/${currentYear()}/${month}/${month}`
      )
    );
    const arrivalTimeFromEmployee: any = [];
  ...
  const confirmationUpdateEmployee = (value: boolean) => {
    dispatch({
      type: AttendanceRegister.UPDATE_EMPLOYEE_CONFIRMATION_MODAL,
      payload: value,
    });
  };
